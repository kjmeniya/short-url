...
<div class="mock-result-row d-flex align-items-center gap-3 p-3 rounded-3 guest-link-card {{ $extraClasses }} position-relative border-0">
    ...
    <div class="d-flex gap-2 flex-shrink-0">
        <div class="align-items-center gap-2 mt-1 justify-content-center justify-content-sm-start flex-wrap d-none d-sm-flex">
            <span class="text-muted text-sm small"><i data-lucide="bar-chart-2" class="icon-sm"></i> {{ number_format($link->clicks ?? 0) }} </span>
            <span class="text-muted text-sm small"><i data-lucide="clock" class="icon-sm"></i> {{ $link->created_at ? $link->created_at->diffForHumans() : '' }}</span>
        </div>
        <div class="d-flex gap-2 flex-shrink-0">
    ...
    <span class="badge bg-{{ $statusColor }} bg-opacity-15 position-absolute top-0 end-0">
