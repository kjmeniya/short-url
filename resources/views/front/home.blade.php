  ...
  <div class="container position-relative z-1">
    <div class="row justify-content-center text-center mb-5">
      <div class="col-lg-8 mb-5">
        ...
        <p class="lead text-muted mb-4 animate-fade-in-up-delay-2 hero-subtitle mx-auto">
          {{ site_description() }} Transform long, ugly links into clean, trackable URLs that drive more clicks and better results.
        </p>
        <div class="d-flex flex-column flex-sm-row gap-3 justify-content-center animate-fade-in-up-delay-3">
    ...
    <div class="row justify-content-center align-items-stretch gy-4 animate-fade-in-up-delay-3">

      <!-- App Body Column -->
      <div class="col-lg-6 text-center position-relative transition-all" id="heroLeftCol">
        <div class="hero-image-wrapper h-100 d-flex flex-column">
          ...
          {{-- Browser chrome card --}}
          <div class="shortener-mock-card glass-card rounded-4 shadow-lg position-relative z-2 p-0 overflow-hidden text-start d-flex flex-column bg-white">
            ...
            {{-- App body --}}
            <div class="p-4 pt-4 flex-grow-1">
              ...
              <div id="heroResultPanel" class="d-none hero-result-panel mt-3">
                <div class="d-flex align-items-center gap-2 my-3">
                  <div class="flex-grow-1" style="height:1px;background:rgba(var(--bs-primary-rgb),.12);"></div>
                  <span style="font-size:.7rem;opacity:.5;font-weight:600;letter-spacing:.05em;" class="text-primary">YOUR SHORT LINK</span>
                  <div class="flex-grow-1" style="height:1px;background:rgba(var(--bs-primary-rgb),.12);"></div>
                </div>

                <div class="mock-result-row d-flex align-items-center gap-3 p-3 rounded-3 flex-column flex-sm-row mx-auto" style="max-width: 100%;">
                  ...
                  <div class="flex-grow-1 text-center text-sm-start overflow-hidden w-100">
                ...
                <div class="d-flex gap-2 flex-wrap mt-3 justify-content-center justify-content-sm-start">
            ...
            <div class="d-flex gap-3 my-3 justify-content-center flex-wrap position-relative z-2">
              <div class="mock-ticker-pill bg-white shadow-sm border-0">
      ...
      <!-- Recent Short Links Column -->
      <div id="guestLinksSection" class="col-lg-6 transition-all @if(!isset($guestLinks) || $guestLinks->isEmpty()) d-none @endif">
        <div class="card border-0 rounded-4 shadow-lg overflow-hidden glass-card flex-column position-relative z-2 h-100">

          {{-- Header --}}
          <div class="card-header p-3 px-4 d-flex align-items-center justify-content-between flex-wrap gap-3 bg-white border-bottom">
            <div class="d-flex align-items-center gap-3">
              <div class="d-flex align-items-center justify-content-center bg-primary bg-opacity-10 text-primary rounded-3 flex-shrink-0" style="width:36px; height:36px;">
                <i data-lucide="link" class="icon-md"></i>
              </div>
              <div>
                <h5 class="fw-bold mb-0">Recent Short Links</h5>
                <p class="text-muted small mb-0 mt-1" style="font-size: 0.8rem; line-height: 1.2;">
                  @auth
                  Your recent links. <a href="{{ route('user.dashboard') }}" class="text-primary text-decoration-none fw-medium">View Dashboard</a>.
                  @else
                  Saved locally. <a href="{{ route('user.register') }}" class="text-primary text-decoration-none fw-medium">Create account</a> to keep them.
                  @endauth
                </p>
              </div>
            </div>
          ...
          {{-- List Body --}}
          <div class="card-body p-3 px-4 bg-white flex-grow-1 overflow-auto">
            <div id="guestLinksList" class="d-flex flex-column gap-3 h-100">
              @isset($guestLinks)
              @foreach($guestLinks->take(3) as $link)
              <div class="w-100">
                <x-guest-link-card :link="$link" class="shadow-sm border border-light w-100 bg-body-tertiary hover-elevate transition-all" />
              </div>
              @endforeach
              @endisset
            </div>
          ...
          {{-- Footer --}}
          <div class="card-footer p-3 px-4 d-flex flex-column flex-sm-row align-items-center justify-content-between gap-3 bg-white border-top mt-auto" id="guestLinksFooter">
            <div class="d-flex align-items-center gap-2">
              <div class="d-inline-flex align-items-center justify-content-center bg-light text-muted rounded-circle flex-shrink-0" style="width:32px; height:32px;">
                <i data-lucide="info" style="width:16px; height:16px;"></i>
              </div>
              <p class="text-muted small mb-0" id="guestLinksFooterText">
                Showing <span class="fw-bold" id="guestLinksCountFooter">{{ isset($guestLinks) ? min($guestLinks->count(), 3) : 0 }}</span> of <span class="fw-bold" id="guestLinksTotalFooter">{{ $totalGuest ?? (isset($guestLinks) ? $guestLinks->count() : 0) }}</span> links.
              </p>
            </div>

            <div class="d-flex align-items-center gap-2">
              <button type="button" id="viewAllLinksBtn" data-bs-toggle="modal" data-bs-target="#guestLinksModal" class="btn btn-light btn-sm rounded-pill px-3 fw-medium border shadow-sm transition-all text-nowrap">
                View All
              </button>
            </div>
          </div>
