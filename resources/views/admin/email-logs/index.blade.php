...
@section('title', $title ?? 'Email Logs')
@section('description', $description ?? 'View and manage all email logs, track email delivery status, and monitor email performance.')
@section('keywords', $keywords ?? 'email logs, email tracking, email delivery, email monitoring')
...
<div class="card mb-3">
  <div class="card-header p-0">
    <button class="btn btn-sm btn-link w-100 text-start d-flex justify-content-between align-items-center p-3"
      ...
      <span>
        <i data-lucide="bar-chart-3" class="icon-sm me-2"></i>Email Statistics
      </span>
  ...
  <div class="collapse show" id="statisticsCollapse">
    <div class="card-body">
      <div class="row">
          ...
          <div class="card stats-card sent">
            <div class="card-body p-3">
              <div class="d-flex justify-content-between align-items-center">
                ...
                <i data-lucide="send" class="icon-lg text-primary"></i>
              </div>
            </div>
          </div>
        </div>
        ...
        <div class="col-6 col-sm-6 col-md-4 col-lg-4 col-xl-3 col-xxl-2 mb-3">
          <div class="card stats-card opened">
            <div class="card-body p-3">
              <div class="d-flex justify-content-between align-items-center">
                <div>
                  <h6 class="card-title mb-0">Opened</h6>
                  <h4 class="mb-0">{{ number_format($stats['opened']) }}</h4>
                </div>
    ...
    <div class="card">
      <div class="card-body">
        <div class="d-flex flex-wrap justify-content-between align-items-center mb-3 pb-3 border-bottom gap-2">
          <h6 class="card-title mb-0">Email Logs Management</h6>
            ...
            <div class="card-body">
              <div class="row g-3">
                <div class="col-12 col-sm-6 col-md-3 mt-0 mb-2">
                  ...
                  <input type="text" id="customSearchBox" class="form-control form-control-sm" placeholder="Search emails...">
                </div>
                <div class="col-12 col-sm-6 col-md-3 mt-0 mb-2">
    ...
    <div class="modal-content">
      ...
      <div class="modal-body">
        <div class="row mb-3">
          ...
          <div class="col-md-6">
            <strong>From:</strong> <span id="previewSender"></span>
          </div>
        </div>
        <div class="row mb-3">
          <div class="col-12">
...
<script>
  $(document).ready(function() {
    ...
    // Filter handlers
    $('#statusFilter, #typeFilter, #templateFilter').on('change', function() {
      table.draw();
      updateActiveFiltersCount();
    });
    ...
    $('#exportLogs').on('click', function() {
      const params = new URLSearchParams({
        ...
        type: $('#typeFilter').val(),
        template_id: $('#templateFilter').val()
      });
      ...
      window.location.href = "{{ route('admin.email-logs.export') }}?" + params.toString();
    });
  ...
  function previewEmail(emailLogId) {
    $.get("{{ route('admin.email-logs.preview', ':id') }}".replace(':id', emailLogId))
      .done(function(data) {
        $('#previewRecipient').text(data.recipient);
        $('#previewSender').text(data.sender);
  ...
  function retryEmail(emailLogId) {
    Swal.fire({
      title: 'Retry Email?',
      ...
      if (result.isConfirmed) {
        $.post("{{ route('admin.email-logs.retry', ':id') }}".replace(':id', emailLogId), {
            _token: '{{ csrf_token() }}'
          })
          .done(function(data) {
            Swal.fire({
