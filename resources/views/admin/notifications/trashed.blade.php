...
@section('title', $title ?? 'Deleted Notifications')
@section('description', $description ?? 'Manage soft-deleted notifications with restore options')
@section('keywords', $keywords ?? 'deleted notifications, soft delete, restore notifications, trashed notifications')
...
<nav class="page-breadcrumb">
  <ol class="breadcrumb">
    ...
    <li class="breadcrumb-item active" aria-current="page">Deleted Notifications</li>
  </ol>
</nav>
    ...
    <div class="card">
      <div class="card-body">
        <div class="d-flex flex-column flex-sm-row justify-content-between align-items-start align-items-sm-center mb-3 pb-3 border-bottom gap-2">
          <h6 class="card-title mb-0">Deleted Notifications Management</h6>
          <div>
            <a href="{{ route('admin.notifications.index') }}" class="btn btn-primary btn-sm">
              <i data-lucide="arrow-left" class="icon-sm me-1"></i>
              <span class="d-none d-sm-inline">Back to Notifications</span>
        ...
        <!-- Deleted Notifications Filters Section -->
        <div class="card mb-3">
            ...
            <div class="card-body">
              <div class="row g-3">
                <div class="col-12 col-sm-6 col-md-3 mt-0 mb-2">
                  ...
                  <input type="text" id="customSearchBox" class="form-control form-control-sm" placeholder="Search deleted notifications...">
                </div>
                <div class="col-12 col-sm-6 col-md-3 mt-0 mb-2">
        ...
        <div class="table-responsive mb-3">
          <table id="trashedNotificationsTable" class="table table-hover">
            <thead>
              <tr class="bg-dark">
                <th>Icon</th>
                <th>Title</th>
...
<script>
  $(document).ready(function() {
    ...
    var table = $('#trashedNotificationsTable').DataTable({
      processing: true,
      serverSide: true,
      ajax: {
        url: "{{ route('admin.notifications.trashed') }}",
        data: function(d) {
      ...
      dom: 'rt',
      language: {
        emptyTable: "No deleted notifications found"
      }
    });
    ...
    $('#typeFilter, #deletedByFilter').on('change', function() {
      updateActiveFiltersCount();
      table.ajax.reload();
    });
    ...
    $('#clearFilters').on('click', function(event) {
      ...
      $('#typeFilter, #deletedByFilter').val('');
      $('#customSearchBox').val('');
      dateFilter.clear();
      updateActiveFiltersCount();
    ...
    $(document).on('click', '.restore-notification', function(e) {
      e.preventDefault();
      var notificationId = $(this).data('id');

      Swal.fire({
        title: 'Are you sure?',
        text: "This will restore the notification!",
        icon: 'question',
        ...
        if (result.isConfirmed) {
          $.ajax({
            url: "{{ route('admin.notifications.restore', ':id') }}".replace(':id', notificationId),
            type: 'POST',
            data: {
              _token: '{{ csrf_token() }}'
            },
            ...
            error: function(xhr) {
              var errorMessage = 'An error occurred while restoring the notification.';
    ...
    $(document).on('click', '.force-delete-notification', function(e) {
      e.preventDefault();
      var notificationId = $(this).data('id');

      Swal.fire({
        title: 'Are you absolutely sure?',
        html: '<div class="text-danger"><strong>This action cannot be undone!</strong></div><br>This will permanently delete the notification.',
        icon: 'warning',
        showCancelButton: true,
        ...
        if (result.isConfirmed) {
          $.ajax({
            url: "{{ route('admin.notifications.force-delete', ':id') }}".replace(':id', notificationId),
            type: 'DELETE',
            data: {
              _token: '{{ csrf_token() }}'
            },
            ...
            error: function(xhr) {
              var errorMessage = 'An error occurred while permanently deleting the notification.';
              if (xhr.responseJSON && xhr.responseJSON.message) {
                errorMessage = xhr.responseJSON.message;
              }
              Swal.fire({
